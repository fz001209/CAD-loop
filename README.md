...
### 强制规则
操作顺序: 先草图(sketch_*) → 再拉伸(extrude/cut_extrude)

布尔运算: union/cut/intersect必须提供args.a_id和args.b_id

特征操作: 必须有可操作的实体引用

唯一标识: 每个操作必须有id，便于调试和修补
...
调试建议
优先查看执行轨迹: exec.log.txt中的op_trace是最直接的调试信息

检查几何指标: 异常的bbox尺寸通常意味着参数单位混乱

查看渲染证据: 验证器依赖PNG图像，确保渲染成功

利用patch机制: Agent5生成的unified diff可以精确修复IR问题
